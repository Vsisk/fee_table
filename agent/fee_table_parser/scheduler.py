...
from agent.fee_table_parser.event_schema import CATEGORY_EVENT_TYPES, COLUMN_EVENT_TYPES
...
class FeeTableCategoryScheduler:
    ...
    async def run(self, source_view: FeeTableSourceView, reducer: FeeTableStreamReducer) -> None:
        ...
        async with self._lock:
            reducer.consume_raw_jsonl(column_jsonl, allowed_event_types=COLUMN_EVENT_TYPES)
        ...
        async with self._lock:
            result = reducer.consume_raw_jsonl(category_jsonl, allowed_event_types=CATEGORY_EVENT_TYPES)
